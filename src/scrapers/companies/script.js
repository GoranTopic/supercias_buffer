...
let scrap_informacion_general = async (page, path) => {
		/* scrap documetns  */
		console.log('scrap Información geneal ran')
		// add tab name
...
for( let current_tab of tabs ){
    ...
    let current_tab_name = sanitize( await getText(current_tab) );
    // if it has not been checked off
    if( ! checklist_tabs.isCheckedOff( current_tab_name ) ){
        console.log('checked off tab ran')
        debugger;
        // if we hae function for it 
        console.log(tab_scrapers);
        console.log(current_tab_name);
        console.log(tab_scrapers[current_tab_name]);
        if( tab_scrapers[current_tab_name] ){
            // if tab has an associated function
            try{
                // click on first tab 
                await waitUntilRequestDone(page, 1000);
